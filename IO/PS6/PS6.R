...
f_dist <- approxfun(f_indsym$x, f_indsym$y, yleft=0, yright=1)
#f_cdf <- apply(cbind(-Inf, f_indsym$x), 1, function(x) {integrate(f_dist, lower=x[1], upper=x[2])$value}) 
f_cdf <- ecdf(unlist(stack(fpa, select=c("u1", "u2", "u3", "u4"))[1]))
...
plot(f_cdf,lwd=5,  main ="Empirical CDF of valuations assuming independence and symmetry \n (plotted with individual marginals for comparison)")
lines(cdf_1, type='l', cex=0.2, col="orange")
lines(cdf_2, col="red", cex=0.2)
lines(cdf_3, col="blue", cex=0.2)
lines( cdf_4, col="green", cex=0.2)
...
g <- density(unlist(stack(fpa, select=c("V1", "V2", "V3", "V4"))[1]), from=0, to=max(grid))
g_dist <- approxfun(g$x, g$y, yleft=0, yright=0)
G_vals <- apply(cbind(-Inf, g$x), 1, function(x) {integrate(g_dist, lower=x[1], upper=x[2])$value})
...
f_SIPV <- density(unlist(stack(fpa, select=c("uhat_1", "uhat_2", "uhat_3", "uhat_4"))[1]), from=0, to=max(350))
f_SIPV_int <- approxfun(f_SIPV$x, f_SIPV$y, yleft=0, yright=0)
F_SIPV <- ecdf(unlist(stack(fpa, select=c("uhat_1", "uhat_2", "uhat_3", "uhat_4"))[1]))
...
plot(f_SIPV, lwd=3, main ="Estimated marginal distribution of valuations under initial SIPV assumption \n (plotted with previous marginal for comparison)")
...
plot(F_SIPV, xlim=c(0,max(u_grid)),col="black", lwd=3, main="Empirical CDF of valuations under initial SIPV assumption \n (plotted with previous CDF for comparison)")
lines( f_cdf, lwd=3, col="red")
