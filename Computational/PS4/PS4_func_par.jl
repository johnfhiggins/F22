...
mutable struct Results
    val_func_next_t :: SharedArray{Float64,3} #value function struct for value function in next period
    val_func ::SharedArray{Float64, 3} #value function for current period
    cap_pf :: SharedArray{Float64,3} #capital policy function struct
    labor_pf :: SharedArray{Float64,3} #labor supply policy function struct
    μ :: Vector{Float64} #size of cohorts by age
    Γ :: SharedArray{Float64,3} # dist of agents over age, prod, and assets
    Γ_0 :: SharedArray{Float64,3} #initial ss dist of agents over age, prod, and assets
    K :: Float64 #aggregate capital supply
    L :: Float64 #aggregate labor supply
    cap_pf_path :: SharedArray{Float64, 4}
    lab_pf_path :: SharedArray{Float64, 4}
    k_path_guess :: SharedArray{Float64,1}
    l_path_guess :: SharedArray{Float64,1}
end
...
function utility_w(ap::Float64, a::Float64, prod::Float64, prim::Primitives, param::Param )
    ...
    @unpack γ = param
    u = 0.0
    if work_budget(ap, a, prod, param) > max(0.0,ap) && (opt_l(ap, a, prod, param) < 1 || γ == 1.0)
...
function cons_opt(prim::Primitives, res::Results, param::Param)
    ...
    for age in reverse(jr:N-1)#loop backwards over retirement ages
        ...
        @sync @distributed for a_i=1:na
            ...
            val_opt = optimize(ap ->  -utility_r(ap, A[a_i], budget, prim, param) - β*vf_interp[1](ap) , a_min, min(max(budget, a_min + 0.01), a_max))
            ...
            res.val_func[age, a_i, :] .= utility_r(closest_ap, A[a_i], budget, prim, param) + β*vf_interp[1](closest_ap)
    ...
    for age=reverse(1:jr-1)#loop backwards over working ages
        ...
        @sync @distributed for a_i=1:na #iterate over asset holdings
            a = A[a_i]
...
function Γ_finder(prim::Primitives, res::Results, param::Param)
    ...
    μ = res.μ
    Γ = SharedArray{Float64,3}(zeros(N, na, 2))
...
function Γ_non_stat(prim::Primitives, res::Results, param::Param)
    ...
    μ = res.μ
    Γ_new = SharedArray{Float64,3}(zeros(N, na, 2))
...
function path_finder(prim::Primitives, res::Results, T_g::Int64)
    ...
    param = init_param(θ_seq = θ_seq_T, w=1.05, r = 0.05, b = 0.2, Z = [3.0, 0.5], γ = 0.42, t =T_g, T = T_g)
    K_T, L_T, w_T, r_T, b_T = kl_search(prim, res, param) #search for SS without social security at time T
    ...
    while error > tol && n <= 1000
        ...
        n+=1
        res.k_path_guess = 0.9*res.k_path_guess + 0.1*K_path
        res.l_path_guess = 0.9*res.l_path_guess + 0.1*L_path
...
function path_finder(prim::Primitives, res::Results, T_g::Int64, θ_seq_mod::Vector{Float64})
    ...
    res.Γ_0 = res.Γ
    #θ_seq_T = vcat([0.11],  fill(0.0, T_g))
    param = init_param(θ_seq = θ_seq_mod, w=1.05, r = 0.05, b = 0.2, Z = [3.0, 0.5], γ = 0.42, t =T_g, T = T_g)
    K_T, L_T, w_T, r_T, b_T = kl_search(prim, res, param) #search for SS without social security at time T
    #res.val_func_next_t = res.val_func
    res.k_path_guess, res.l_path_guess = path_guess(k_0, l_0, K_T, L_T, T_g) #create initial guess for sequence of aggregate capital and labor which gets us within tolerance
    K_path, L_path, error = path_compare(prim,res, param, k_0, K_T, T_g) #find max error between guessed path and first iteration
    n = 0
    while error > tol && n <= 1000
        println("Iteration $(n), error = $(error)")
...
function ce_meas_func(prim, λ, Γ_0)
    @unpack na, N = prim
    EV = zeros(N)
    for age=1:N
...
function ce_avg_a(prim, λ, Γ_0)
    @unpack na, N = prim
    EV = zeros(N)
    for age=1:N
        EV[age] =  sum(λ[age, :,:] .* Γ_0[age, :,:])/sum(Γ_0[age,:,:])#(sum(λ[age, :, 1] .* Γ_0[age, :, 1]) + sum(λ[age, :, 2] .*Γ_0[age, :, 2]))/(sum(Γ_0[age, :, 1 ] + Γ_0[age, :, 2]))
    end
    EV
end
