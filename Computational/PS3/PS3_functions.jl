    ...
    for age in reverse(jr:N-1)#loop backwards over retirement ages
        ...
        for a_i=1:na
            ...
            for ap_i=1:na
                ...
                if c >= 0
                    val = (c)^((1-σ)*γ)/(1-σ) + β*v_next[age+1, ap_i, 1]
    ...
    for age=reverse(1:jr-1)#loop backwards over working ages
        ...
        for a_i=1:na
            for z_i = 1:2
                ...
                for ap_i= 1:na
                    prod = η*Z[z_i]
                    opt_l = max(0,min(1,(γ*(1-θ)*prod*w - (1-γ)*((1+r)*A[a_i] - A[ap_i]))/((1-θ)*w*prod)))
                    res.labor_pf[age, a_i, z_i] = opt_l
                    budget = w*(1-θ)*prod*opt_l + (1+r)*A[a_i]
                    c = budget - A[ap_i] #amount left over for consumption
                    if c >= 0
                        val = (((c)^γ *(1-opt_l)^(1-γ))^(1-σ))/(1-σ) 
                        for zp_i=1:2
                            val += β*v_next[age+1, ap_i, zp_i]*Π[z_i, zp_i]
                        end
                        if val < val_prev
                            break
                        else
